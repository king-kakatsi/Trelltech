...
export const createBoard = async (
  ...
  try {
    ...
    let endpoint = `/boards/?key=${
      TRELLO_CONFIG.API_KEY
    }&token=${token}&name=${encodeURIComponent(
      name
    )}&idOrganization=${workspaceId}&defaultLists=false`;
...
export const updateBoardName = async (boardId, newName) => {
  try {
    const token = await fetchFromLocalStorage("trello_token");
    if (token) {
      const endpoint = `/boards/${boardId}?key=${
        TRELLO_CONFIG.API_KEY
      }&token=${token}&name=${encodeURIComponent(newName)}`;
...
export const createList = async (boardId, listName, pos = "bottom") => {
  try {
    const token = await fetchFromLocalStorage("trello_token");
    if (token) {
      const endpoint = `/lists?key=${
        TRELLO_CONFIG.API_KEY
      }&token=${token}&name=${encodeURIComponent(
        listName
      )}&idBoard=${boardId}&pos=${pos}`;
...
export const updateList = async (listId, newName) => {
  try {
    const token = await fetchFromLocalStorage("trello_token");
    if (token) {
      const endpoint = `/lists/${listId}?key=${
        TRELLO_CONFIG.API_KEY
      }&token=${token}&name=${encodeURIComponent(newName)}`;
